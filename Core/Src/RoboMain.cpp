...
void RobotMoveSpinRight(double speed, double ForwardDis) //ForwardDis单位为cm//位置环控制
{
    ForwardDis = ForwardDis / 30 * 60000;
...
void VofaTransmitInc(){
    char bufferLRFB[128];
    sprintf(bufferLRFB, "%f,%f,%f,%f \n", encodingMotorLF.pid.GetInput(),encodingMotorRF.pid.GetInput(),encodingMotorLB.pid.GetInput(),encodingMotorRB.pid.GetInput());
    ...
    while(rx_times_flag1<=800)
    {
        ...
        if (k210_msg_flag == 1)
        {
                ...
                k210_msg[16] == '8' && k210_msg[15] == '8' && k210_msg[14] == '8' && k210_msg[13] == '8')
            {
                if (k210_msg[11] == '0' && (k210_msg[12] == '1' || k210_msg[12] == '2'))
                    fruit_flag = 1;
                if (k210_msg[11] == '0' && (k210_msg[12] == '4' || k210_msg[12] == '5'))//没有识别到
                    break;
...
void RobotReceivedFruitVoice() {
    ...
    {
        // 数据格式为 "Distance:xxxx"
        char *distance_ptr = strstr((char *)rx_buffer, "Distance: ");   //查找"Distance:"字符串
        if (distance_ptr != NULL)
        {
            uint16_t distance;
            sscanf(distance_ptr, "Distance: %hu", &distance);   //将字符串后面的数字储存在distance中

            if (distance <= 90){
                RobotBeginVoice(0, (uint8_t *)"[v12][m0][t5]抓取到水果一个");
                HAL_Delay(2000);
            }
            else{
                RobotBeginVoice(0, (uint8_t *)"[v12][m0][t5]没有抓取到水果");
                HAL_Delay(2000);
            }
        }
    }
...
void RobotReceivedVegetableVoice() {
    ...
    if (HAL_UART_Receive(&huart3, rx_buffer, sizeof(rx_buffer), 1000) == HAL_OK)
    {
        ...
        if (distance_ptr != NULL)
        {
            ...
            if (distance <= 80){
                RobotBeginVoice(0, (uint8_t *)"[v12][m0][t5]抓取到蔬菜一个");
                HAL_Delay(2000);
            }
            else{
                RobotBeginVoice(0, (uint8_t *)"[v12][m0][t5]没有抓取到蔬菜");
                HAL_Delay(2000);
            }
        }
    }
