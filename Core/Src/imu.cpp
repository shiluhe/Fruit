...
void IMU::handleIMUPackage(int cmd, uint8_t *data, int len) {
    if (cmd == 0x40) {
        auto* msgImu = (struct MSG_IMU*)data;//直接进行解码操作
...
int IMU::decodeAHRSPackage(uint8_t *data) {   //data长度为1024, 输入的数组大小一定为280，不然指针会丢失。
    //定位帧头
    int packageStart = 0;
    int flag = 0;
    ...
    if (data[0] == 0xFC && crc8 == data[4]) { //是帧头
        int cmd = data[1];  //指令类别
        int dataLen = data[2]; //数据长度

        //判断是否接收完整个数据 本程序一定接收完整
